...
export function netPointsDiamond(
  netpoints: { tm: string }[],
  options: {
    by: string
    percentile: number
  ...
  let data = sliceQuantile(netpoints, by, (100 - percentile) / 100)
  if (selectedTeams?.length > 0) {
    data = data.filter(d => selectedTeams.includes(d.tm))
  }
  console.log(data, selectedTeams, options)

  // Used if the "show rest of NBA" option is selected, to show the rest of the
  // league as background data
  const background = sliceQuantile(netpoints, by, (100 - percentile) / 100)
  ...
  const epmplot = Plot.plot({
    ...
    marks: [
      ...
      Plot.dot(data, {
        x,
        y,
        fill: d => getTeam(d.tm).colors[0],
        stroke: d => getTeam(d.tm).colors[1],
        title: d => `${d.full_nm}\n${d.tm}\n${x}: ${d[x]}\n${y}: ${d[y]}`,
        r: 8,
      }),
    ],
  ...
  container
    ...
    .style("top", "60px")
    .style("left", "10px")
    .html(`Top ${percentile}% by minutes<br>Data by espnanalytics.com`)
  container
    .append("div")
    .text("← Better defense")
    .style("position", "relative")
